...
fun Iterable<RERule>.applyAllRules(acmd: String, log: IMyLogger): String {
    var cmd = acmd
    for (rule in this) cmd = rule.apply(cmd, log)
    return cmd
}
...
class REGroup(
    ...
    val rules: MutableList<RERule> get() = if (editable) mRules else Collections.unmodifiableList(mRules)
...
fun Iterable<REGroup>.applyAllGroups(acmd: String, log: IMyLogger): String {
    var cmd = acmd
